...
today, it's about 500GGB of data. The data has an elementary index that
...
* Indexer is reading the stream of traces and deltas from Chronicle and
  ...
  multiple back-end instances.

* Query processing. The API requests would search for relevant block
  numbers in the indexer database, and then corresponding blocks would
  be retrieved by Chronicle from the state history archive. This process
  can be easily distributed acrtoss multiple servers and multiple
  processes to occupy CPU cores efficiently. Once the blocks are
  retrieved and decoded into JSON data, this data will be cached in
  RAM. Least Recently Used (LRU) cache will allow efficient re-use of
  data that is requested more often (for example, eosio.token transfers
  in the latest few hundred blocks).
